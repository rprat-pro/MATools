...
bool run_test_elasticity()
{
  ...
  for(int id = 0 ; id < s*size ; id++)
  {
    ...
    {
      if( std::abs(host[id] - 3.46154e+11) >= 1e+6) // e+6 -> precision
      {
	std::cout << " error id: " << id << " host: " << host[id] << std::endl;  
	return EXIT_FAILURE;
      }
...
#ifdef __CUDA__
  success &= run_test_elasticity<double, MEM_MODE::CPU, GPU_TYPE::CUDA>();
  success &= run_test_elasticity<double, MEM_MODE::GPU, GPU_TYPE::CUDA>();
#endif
