...
std::vector<T> get_vec_data(MAGPUVector<T, MM, GT> a_vec);
*/
...
#define MY_TEST_CASE(NAME,TYPE,MEMORY,GPUTYPE) TEST_CASE(QUOTE test_[NAME]_[TYPE]_[MEMORY]_[GPUTYPE] QUOTE,"[MAGPU]") \
{\
  using namespace MATools::MAGPU;\
...
#define GPU_TEST_CASE(NAME,TYPE,MEMORY) \
  MY_TEST_CASE(NAME, TYPE, MEMORY, MATools::MAGPU::GPU_TYPE::SERIAL)\
  MY_TEST_CASE(NAME, TYPE, MEMORY, MATools::MAGPU::GPU_TYPE::CUDA)
